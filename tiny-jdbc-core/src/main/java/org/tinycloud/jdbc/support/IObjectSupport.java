...
public interface IObjectSupport<T, ID extends Serializable> {
     ...
     *     <li>返回数组的长度与传入集合长度相同。{@code JDBC_BATCH} 模式下每个元素是对应实例的真实受影响行数；
     *         {@code MULTI_VALUE} 模式下为单条多值 INSERT 的语句级影响行数（同一语句内各元素相同），
     *         需要逐行结果请改用 {@code JDBC_BATCH} 模式。</li>
     * </ol>
     *
     * @param collection 待插入的实例集合，不能为空或空集合
     * @return 一个数组，元素语义见「注意事项」中的模式说明
     ...
     * <h3>注意事项：</h3>
     * <ol>
     *     <li>返回的数组长度与传入的实例集合长度相同。{@code JDBC_BATCH} 模式下每个元素是对应实例的真实受影响行数；
     *         {@code MULTI_VALUE} 模式下为单条多值 INSERT 的语句级影响行数（同一语句内各元素相同），
     *         需要逐行结果请改用 {@code JDBC_BATCH} 模式。</li>
