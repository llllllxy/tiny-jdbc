...
/**
 * {@link SqlGenerator} 按主键查询/删除 SQL 生成的验证：
 ...
 */
public class SqlGeneratorPrimaryKeyVerifyMain {
    ...
    public void testSelectByIdSql() {
        SqlProvider so = SqlGenerator.selectByIdSql(100L, PrimaryKeyEntity.class);
    ...
    public void testSelectByIdsSql() {
        SqlProvider so = SqlGenerator.selectByIdsSql(PrimaryKeyEntity.class, Arrays.<Object>asList(1L, 2L));
    ...
    public void testDeleteByIdSql() {
        SqlProvider so = SqlGenerator.deleteByIdSql(100L, PrimaryKeyEntity.class);
    ...
    public void testDeleteByIdsSql() {
        SqlProvider so = SqlGenerator.deleteByIdsSql(PrimaryKeyEntity.class, Arrays.<Object>asList(1L, 2L));
    ...
    public void testMissingPrimaryKeyThrowsExplicitError() {
        ...
        assertThrows(() -> SqlGenerator.selectByIdSql(1L, NoPkEntity.class), "selectByIdSql should reject missing @Id");
        assertThrows(() -> SqlGenerator.selectByIdsSql(NoPkEntity.class, Arrays.<Object>asList(1L)), "selectByIdsSql should reject missing @Id");
        assertThrows(() -> SqlGenerator.deleteByIdSql(1L, NoPkEntity.class), "deleteByIdSql should reject missing @Id");
        assertThrows(() -> SqlGenerator.deleteByIdsSql(NoPkEntity.class, Arrays.<Object>asList(1L)), "deleteByIdsSql should reject missing @Id");

        // 条件构造器默认列（未显式指定 select 字段）同样保留原有主键校验
        assertThrows(() -> SqlGenerator.selectCriteriaSql(new QueryCriteria<NoPkEntity>(), NoPkEntity.class), "selectCriteriaSql should reject missing @Id");
        assertThrows(() -> SqlGenerator.selectLambdaCriteriaSql(new LambdaQueryCriteria<NoPkEntity>(), NoPkEntity.class), "selectLambdaCriteriaSql should reject missing @Id");
    }
